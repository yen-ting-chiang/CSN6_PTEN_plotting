# 可選：設定你的工作資料夾
setwd("C:/Users/danny/Documents/R_project/CSN6_PTEN_plotting/fig_5f_colony_1")

# =========================================================
# Nature journal style barplot for colony formation assay
# - 讀取 fig_5f_colony_1_cleaned.xlsx（欄：CMV, PTEN, PTEN+CSN6）
...
# ---- 0) 基本設定 ----
input_path  <- "fig_5f_colony_1_cleaned.xlsx"   # 若放在 /mnt/data，改成 "/mnt/data/fig_5f_colony_1_cleaned.xlsx"
sheet_name  <- 1
y_label     <- "Colony formation (a.u.)"        # 若是計數，可改為 "Colony count"
figure_tag  <- "Fig_5F_ColonyFormation_barplot" # 會自動加上 _SD / _SEM
...
expected_cols <- c("CMV5","PTEN","PTEN+CSN6","CSN6")
...
comparisons <- combn(c("CMV5","PTEN","PTEN+CSN6","CSN6"), 2, simplify = FALSE)
...
make_barplot <- function(summary_df, long_df, ttest_df,
  ...
  # y 上界：若不畫 p 值，留一點空白；若畫，依照註記行數自動墊高
