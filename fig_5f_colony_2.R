# 可選：設定你的工作資料夾
setwd("C:/Users/danny/Documents/R_project/CSN6_PTEN_plotting/fig_5f_colony_2")

# =========================================================
# Nature journal style barplot for colony formation assay (version 2)
# - 讀取 fig_5f_colony_2_cleaned.xlsx（欄：CMV5, PTEN, PTEN+CSN6, CSN6）
# - 產生兩種圖：SD 誤差棒 & SEM 誤差棒（圖上不顯示 p 值）
# - 疊加原始點、Welch's t-test（Holm 校正）→ 只輸出 CSV（不在圖上標）
# - 輸出：.tiff (LZW, 600 dpi), .pdf (向量), .jpg (600 dpi)
# - 另輸出 pairwise t 檢定之 raw p-value 與 Holm adjusted p-value 的 CSV
# =========================================================

# ---- 0) 基本設定 ----
input_path  <- "fig_5f_colony_2_cleaned.xlsx"   # 若放在 /mnt/data，改成 "/mnt/data/fig_5f_colony_2_cleaned.xlsx"
sheet_name  <- 1
y_label     <- "Colony formation (a.u.)"        # 若是計數，可改為 "Colony count"
figure_tag  <- "Fig_5F_ColonyFormation2_barplot" # 會自動加上 _SD / _SEM
...
expected_cols <- c("CMV","CSN6","CSN6+PTEN")
...
# ---- 4) 成對比較（Welch’s t-test, Holm 校正；結果輸出 CSV，不畫在圖上）----
comparisons <- list(c("CMV","CSN6"),
                    c("CSN6","CSN6+PTEN"),
                    c("CMV","CSN6+PTEN"))
...
make_barplot <- function(summary_df, long_df, ttest_df,
  ...
  # y 上界：不畫 p 值時留一點空白；若改為 show_p=TRUE 會自動墊高
